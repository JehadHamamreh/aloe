...
function bcch_bch_msg = lte_bcch_bch_msg_pack(bw, phich_dur, phich_res, sfn)
    ...
    elseif (bw == 100)
        ...
        fprintf('ERROR: Invalid bw (%u)\n', bw);
        bcch_bch_msg = 0;
        return;
    end

    % Check phich_dur
    if (phich_dur == 0)     % 0 indicates normal
        act_phich_dur = 0;
    elseif (phich_dur == 1) % 1 indicates extended
        act_phich_dur = 1;
    else
        fprintf('ERROR: Invalid phich_dur (%s)\n', phich_dur);
        bcch_bch_msg = 0;
        return;
    end

    % Check phich_res
    if (phich_res < 0.5)        % 1/6
        act_phich_res = [0, 0];
    elseif (phich_res == 0.5)   % 1/2
        act_phich_res = [0, 1];
    elseif (phich_res == 1)
        act_phich_res = [1, 0];
    elseif (phich_res == 2)
        ...
        fprintf('ERROR: Invalid phich_res (%f)\n', phich_res);
        bcch_bch_msg = 0;
        return;
    end

    % Check SFN (8 bits, the 8 MSBs)
    act_sfn = zeros(1,8);
    if (sfn >= 0 && sfn <= 1023)
        tmp = sfn;
        for n=0:7
