...
int vector_convolution(int numsamples, int decimate, _Complex float *in,\
	...
	if(length > MAXCORRWINDOW){
		moddebug("filter length too long%d\n",length);
		return 0;
	}
...
void quantifyCvectorGO(_Complex float *in, _Complex float *out, int length, int n){
	...
	moddebug("DA converter: number of bits = %d\n", n);
	moddebug("DA converter: Quantification levels = %d\n", (1<<(n-1)));
	for(i=0; i<length; i++){
...
int init_ifft_LTE(int FFTsize, _Complex float *in, _Complex float *out){
	...
	if(FFTsize==64){
		plan64 = fftwf_plan_dft_1d(64, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	if(FFTsize==128){
		plan128 = fftwf_plan_dft_1d(128, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	if(FFTsize==512){
		plan512 = fftwf_plan_dft_1d(512, in,out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	if(FFTsize==1024){
		plan1024 = fftwf_plan_dft_1d(1024, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	if(FFTsize==1536){
		plan1536 = fftwf_plan_dft_1d(1536, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	if(FFTsize==2048){
		plan2048 = fftwf_plan_dft_1d(2048, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
	}
	return 1;
}
