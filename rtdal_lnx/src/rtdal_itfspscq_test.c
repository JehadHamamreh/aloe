...
int rtdal_itfspscq_init(rtdal_itfspscq_t *itf) {
	...
	itf->parent.type = ITF_INT_NONBLOCKING;
	itf->read = 0;
	itf->write = 0;
	...
	if (itf->blocking) {
		sem_init(&itf->sem_r,0,0);
		sem_init(&itf->sem_w,0,0);
	}
...
inline static int spscq_is_empty(rtdal_itfspscq_t *itf) {
	if (!itf->blocking) {
		...
		while(itf->packets[itf->read].valid == 0) {
			qdebug("[read] waiting\n",0);
			sem_wait(&itf->sem_r);
		}
		return 0;
...
int rtdal_itfspscq_send(r_itf_t obj, void* buffer, int len, int tstamp) {
	...
	return rtdal_itfspscq_push(obj,NULL,len,tstamp);
}
