 ...
 */
int synchro(int numsamples, int FFTsize, _Complex float *in, _Complex float *out,\
	...
	N=16;	//NUmber of samples per FFT symbol
	decim = FFTsize / N;
	//modinfo_msg("FFTsize=%d, decim=%d\n", FFTsize, decim);
	// PSS Sequence 0
	...
	for(i=0; i<numblocks; i++){
		...
		if(findMAXFinal(numsamples, out, pMAXFinal, threshold) == 1){
			...
			//printf("PSS_SEQ%d: pMAXFinal= %d, MAXfinal=%3.3f \n", pMAXFinal->seqNumber, pMAXFinal->pMAX, pMAXFinal->MAX);
			//printf("average = %3.3f, max2average=%3.3f, var=%3.3f, max2var=3.3f \n",
			//		pMAXFinal->average, pMAXFinal->max2average, pMAXFinal->var, pMAXFinal->max2var);
...
int detect_CP(int fftpointer, int FFTsize, _Complex float *in){
	...
	modinfo_msg("RCPnorm=%3.1f, PCPnorm=%3.1f, MCPnorm=%3.3f,\n", RCPnorm, PCPnorm, MCPnorm);
	modinfo_msg("RCPext=%3.1f, PCPext=%3.1f, MCPext=%3.3f,\n", RCPext, PCPext, MCPext);
...
void findMAX(int numsamples, int FFTsize, int decimate, _Complex float *corr, int numPSSseq, p2MAX_t *p2MAX, int *numblocks){
	...
	for(j=0; j<*numblocks; j++){
		...
		//modinfo_msg("pMAX[%d]=%d MAX[]=%3.3f average[]=%3.3f max2average=%3.3f var[]=%3.3f max2var=%3.3f\n",
