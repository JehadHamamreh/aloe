...
modulation=1;
sigma2=1;
hard=0; % if 1, soft value is ignored
soft=0;
zero=0;
one=1;
debug=0;
...
if (hard == 1)
    out_mex=am_gen_hard_demod(transpose(x), {{'modulation',int32(modulation)}});
    out_matlab=soft_demapper(x, modulation, 0, zero, one, sigma2, debug);
else
    out_mex=am_gen_soft_demod(transpose(x), {{'soft',int32(1)},{'modulation',int32(2)},{'sigma2',sigma2}}); % soft: 0 means exact LLR and 1 approx. LLR
...
h=plot(1:50,out_matlab(1:50),'b-',1:50,out_mex(1:50),'r--');
set(h,'LineWidth',2.5)
grid on
axis tight;
