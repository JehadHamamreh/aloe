...
function out = scrambling(samples, desc, UL, x, y)
    ...
    addpath('/home/vuk/DATOS/workspace/lte_descrambling');
    ...
    q =         0;
    ns =        0;     % slot. range: integer in [0, 19], default: 0
...
%     channel = 5; % PUCCH
    ...
    if (desc == 0) % scrambling
        ...
        out_mex = am_lte_scrambling(in, {{'q',int32(q)},{'subframe',int32(floor(ns/2))},{'cell_gr',int32(cell_gr)},{'cell_sec',int32(cell_sec)},{'nrnti',int32(nRNTI)},{'channel',int32(channel)},{'direct',int32(direct)}});
        %out_mex = out_mex

    else % descrambling (soft inputs, i.e. floats rather than bits)
