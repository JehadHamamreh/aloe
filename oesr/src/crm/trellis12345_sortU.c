...
//... information for any w: final_map[i] indicates where m[i] is finally mapped
extern int final_map[Mmax];
...
//float tw_mapping(int **ptr_nodes, int w)
{
	...
	for (i=0; i<M; i++)
		final_map[i] = -1; // a final_map[i] = -1 indicates that m[i] is not mapped
	...
	if (cost_t < (infinite - 1)) // feasible mapping
	{
		...
		final_map[M-w] = index;
		for (i=(M-w); i>0; i--)
			final_map[i-1] = indx[final_map[i]][i];
	}
